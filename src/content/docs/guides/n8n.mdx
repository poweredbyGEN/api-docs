...
2. **HTTP Request — Get content engine**
   ...
   ```
   GET https://api.gen.pro/v1/vidsheet/{sheet_id}?agent_id={agent_id}
   ```
...
4. **HTTP Request — Trigger generation**
   ...
   ```
   POST https://api.gen.pro/v1/vidsheet/{sheet_id}/cells/{{ $json.cell_id }}/generate?agent_id={{ $vars.GEN_AGENT_ID }}
   ```
