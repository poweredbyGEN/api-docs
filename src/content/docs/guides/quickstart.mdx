...
curl -s -X POST "https://api.gen.pro/v1/autocontentengine/$ENGINE_ID/cells/$CELL_ID/generate?agent_id=$AGENT_ID" \
  -H "X-API-Key: $GEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"generation_type": "text_generation", "data": {"model": "gemini", "prompt": "..."}}' | jq
...
<LinkCard title="Agent Voice reference" href="/reference/agent-voice/" description="Library, ElevenLabs integration, 4-step voice design, audio cloning, TTS preview." />
