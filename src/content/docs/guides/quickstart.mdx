...
curl -s -X PATCH "https://api.gen.pro/v1/vidsheet/$ENGINE_ID/cells/$CELL_ID?agent_id=$AGENT_ID" \
  -H "X-API-Key: $GEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"value": "Your script here"}' | jq

# d. generate content in the cell (text, image, video, speech — depends on the column type)
curl -s -X POST "https://api.gen.pro/v1/vidsheet/$ENGINE_ID/cells/$CELL_ID/generate?agent_id=$AGENT_ID" \
  -H "X-API-Key: $GEN_API_KEY" \
  -H "Content-Type: application/json" \
...
curl -s -X POST "https://api.gen.pro/v1/vidsheet/$ENGINE_ID/cells/$CELL_ID/render?agent_id=$AGENT_ID" \
  -H "X-API-Key: $GEN_API_KEY" | jq
```
