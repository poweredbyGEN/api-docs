...
<Aside type="tip">
  Source code: [github.com/poweredbyGEN/vidsheet-sdk](https://github.com/poweredbyGEN/vidsheet-sdk)
</Aside>
...
| Image | `image_from_text` | `{ prompt: '...', model: 'gemini_pro', aspect_ratio: '1:1' }` |
| Video | `video_from_text` | `{ prompt: '...', model: 'veo-3.1', duration: 8 }` |
| Video from image | `video_from_image` | `{ prompt: '...', model: 'kling-v2-1', image_resource_id: 123 }` |
| Video from ingredients | `video_from_ingredients` | `{ prompt: '...', images: [{ value: 'https://...' }] }` |
| Speech | `speech_from_text` | `{ text: '...', voice_id: '...' }` |
| Lipsync | `lipsync` | `{ talking_avatar_video_data: { video: { value: 'https://...' }, audio: { value: 'https://...' } }, lip_sync_model: 'gen' | 'sync.so' }` |
| Captions | `captions` | `{ video: { value: 'https://...' } }` |
