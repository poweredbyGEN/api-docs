...
- Exact edit → discover supported fields with `gen_vidsheet_discover`, then call `gen_vidsheet_action`. Read saved state with `gen_get_vidsheet_full`. See [Work with a Vidsheet](/guides/vidsheet-actions/) for draft, generation, and layer examples.
...
| **Build Vidsheets** | Read a sheet with `gen_get_vidsheet_full`, discover card/settings schemas with `gen_vidsheet_discover`, and use `gen_vidsheet_action` for supported structure, drafts, media, and generation |
