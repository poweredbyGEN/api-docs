...
JWTs are issued by Dynamic XYZ during user login. They contain the user's wallet address and are validated against the GEN user database.
...
Firebase real-time streaming is **unauthenticated** — anyone with the `firebase_path` can read progress updates.
