...
| **Agent scope** | All `/v1/vidsheet/` requests require an `agent_id` parameter. You can only use agents you have access to. |
...
Real-time progress streams are addressed by the run-specific `progress_stream_url` returned from `POST /v1/agent/run`. Treat that URL as sensitive application data — it's an opaque channel scoped to a single run. For a fully authenticated path, poll `GET /v1/agent/runs/{run_id}` with your API key instead.
