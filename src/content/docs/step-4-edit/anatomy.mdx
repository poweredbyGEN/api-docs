...
- `gen_vidsheet_action` with `action: "duplicate"` and `target.kind: "row"` — duplicate
...
Every cell has a creation card declaring **how to generate** its content. There are 10 types — [see the full reference](/reference/creation-cards/).
