...
Uploads a new file as a content resource. The file must be sent as a multipart form upload. Optionally attach the resource to a project or place it in an asset folder.
...
| `project_node[project_id]` | integer | No | Attach the resource to this project. |
...
# With project and folder attachment
curl -X POST "https://api.gen.pro/v1/content_resources?agent_id=42" \
  -H "X-API-Key: your-api-key" \
  -F "content_resource[file]=@/path/to/banner.png" \
  -F "project_node[project_id]=100" \
