...
```
GET /v1/vidsheet/{id}/cells/{cell_id}?agent_id={agent_id}
```
...
```bash
curl "https://api.gen.pro/v1/vidsheet/101/cells/3000?agent_id=42" \
...
const response = await fetch(
  "https://api.gen.pro/v1/vidsheet/101/cells/3000?agent_id=42",
  {
    method: "PATCH",
    headers: {
...
```
PUT /v1/vidsheet/{id}/cells/mass_update?agent_id={agent_id}
```
...
```bash
curl -X PUT "https://api.gen.pro/v1/vidsheet/101/cells/mass_update?agent_id=42" \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
...
```
PATCH /v1/vidsheet/{id}/cells/{cell_id}/set_default_user_job?agent_id={agent_id}
```
...
```bash
curl -X PATCH "https://api.gen.pro/v1/vidsheet/101/cells/3000/set_default_user_job?agent_id=42" \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"user_job_id": 789}'
```
