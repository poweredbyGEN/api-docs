---
title: Voice
description: List, design, clone, preview, and manage voices on an agent. Powered by the Agent Core service.
---

import { Aside } from '@astrojs/starlight/components';

The Voice API lives alongside Agent Core under `/v1/agents/{agent_id}/voice/*`. It unifies voice library listing, ElevenLabs integration, prompt-based voice design, audio sample cloning, TTS preview, and voice deletion.

<Aside type="note">
  **Base URL:** `https://api.gen.pro/v1` (production) or `https://staging-api.gen.pro/v1` (staging). Authenticated via `X-API-Key` header.
</Aside>
