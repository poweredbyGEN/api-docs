...
### List the agent's own voices

```
GET /v1/user_voice_resources?agent_id={agent_id}
```

Returns only the voices the user **created** — designed, trained, or cloned — not the shared public catalog. Use `GET /v1/agents/{agent_id}/voice/library` (above) when you want the public and connected-provider voices as well.

```bash
curl "https://api.gen.pro/v1/user_voice_resources?agent_id=$AGENT_ID" \
