...
| `return_all` | boolean | Walk every matching row up to the DW hard cap (1000). Without an explicit limit, lifts the per-call ceiling. |
...
- Every filter you state is honored byte-identically across the fan-out (one DW call per watchlist source).
...
- When `return_all=true` is set without an explicit `limit`, the request walks up to the DW hard cap of 1000 rows.
