...
For programmatic asks where you'd rather build a typed parameter object than a natural-language prompt, the MCP tool `gen_query_watchlist` takes the same ask as structured fields and returns the `run_id` to poll with `gen_get_run_status`.
...
**Response:** a run record with a `run_id`. Poll `GET /v1/agent/runs/{run_id}` until `status == "completed"`; the final answer payload contains a video grid block plus a natural-language summary that respects the filters.
