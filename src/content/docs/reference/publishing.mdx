...
| LinkedIn | `linkedin` | text, one video, 1-9 images, or one document (PDF, PPT, PPTX, DOC, DOCX) | 9 | 3,000 | 400 | Text-only posts allowed. Posts as the connected member. Images cannot be mixed with a video or a document. `linkedin_options`: visibility, alt_text. |
  ...
  "platforms": [
    {
      ...
      "notes": "Posts as the connected member. Text-only, one video, 1-9 images, or one document (PDF, PPT, PPTX, DOC, DOCX — LinkedIn's swipeable document post; title is the document title, max 400). Images cannot be mixed with a video or a document in one post."
    }
  ]
}
```
...
| `linkedin_options` | object | No | **LinkedIn only.** `visibility`: `public` (default), `connections` or `loggedin`. `alt_text`: string[] with one entry per image, in media order (images only). |
...
### LinkedIn with several images and alt text
