...
No authentication required. Each entry has `platform`, `label`, `text_only`, `media` (`video`, `image`, `document`), `max_media_items`, `max_text`, `max_title`, `required`, `optional`, `ignored`, `options` (the platform's options object and the values each field accepts) and `notes`.
