...
**Proof of Genesis settings** control automatic asset backup. They do not upload anything by themselves; vidsheet and assets-page workflows read these settings and call the Proof of Genesis backup endpoint when an eligible asset event happens.

- `proof_of_genesis_enabled` = master switch.
- `proof_of_genesis_auto_backup_sources` = defaults to `downloaded`, `published`; also accepts `generated`, `uploaded`.
- `proof_of_genesis_auto_backup_asset_types` = any of `video`, `image`.

Server-triggered automatic backups can resolve the user's Dynamic Sui wallet and create customer-owned Walrus objects. Manual callers can pass `owner_sui_address` to the Proof of Genesis backup endpoint for the same custody behavior.
...
| `proof_of_genesis_enabled` | Replace |
| `proof_of_genesis_auto_backup_sources` | Replace with any of `generated`, `uploaded`, `downloaded`, `published`. Defaults to `downloaded`, `published`; empty means no automatic backup. |
| `proof_of_genesis_auto_backup_asset_types` | Replace with any of `video`, `image`. |
