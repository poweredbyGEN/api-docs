...
- **Management calls are free.** Each scheduled run bills at execution time against existing `user_jobs` pricing for the relevant `job_type`. If the workspace has no credits when a run fires, the run is skipped and the job remains scheduled — no deletion, no email — and execution resumes automatically when credits return.
...
curl -X POST "https://agent.gen.pro/v1/agents/$AGENT_ID/recurring-jobs" \
  ...
  -d '{
    ...
    "schedule": { "cadence": "weekly", "timezone": "America/Los_Angeles", "time_of_day": "09:00" },
    "delivery": { "type": "email" }
  }'
```
...
| `schedule.cadence` | `"daily" \| "weekly" \| "hourly"` | yes | How often to run. |
...
curl -X PATCH "https://agent.gen.pro/v1/agents/$AGENT_ID/recurring-jobs/$JOB_ID" \
  ...
  -d '{
    ...
    "schedule": { "cadence": "weekly", "timezone": "America/Los_Angeles", "time_of_day": "08:00" }
  }'
```
